...
Static disassembly of 100 KB of 16-bit Borland code desynchronises constantly,
so it cannot answer "which files does this open" reliably. Executing the real
binary can. This provides just enough DOS/BIOS for the game to start up, and
records every interrupt, file access and port write it performs.
...
Usage:
    python trace_dos.py                 # trace the shipped Ducks.exe
    python trace_dos.py --exe Ducks.unpacked.exe
    python trace_dos.py --blaster       # advertise a Sound Blaster
"""
import argparse
import os
import struct
...
PSP_SEG = 0x0100
ENV_SEG = 0x00F0
DIET_SKIP_BLOCKS = 2          # handoff detection, as in unpack_ducks
...
class DosMachine:
    def __init__(self, exe_path, blaster=False, verbose=True, max_insns=80_000_000):
    ...
    def _load(self, path, blaster):
        ...
        env += b"\x00\x01\x00C:\\DUCKS.EXE\x00"
