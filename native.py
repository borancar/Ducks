...
def native_scene_keep_positions(m, args):
    ...
    at +0x0c/+0x10 - and this makes the copy. run_level calls it on five
    of the six scenes before anything moves, so what is at +0x0c is where the
...
def native_entity_set_type(m, args):
    ...
    run_level calls it once a frame on the duck.
    """
...
def native_bg_scroll_reset(m, args):
    ...
    run_level's only caller. The level carries one byte at [0x202c] and it
    is two base-3 digits: the remainder gives the horizontal drift and the quotient
...
def native_scroll_axis_snap(m, args):
    ...
    span), and no mask, because there is nothing to converge to. run_level
